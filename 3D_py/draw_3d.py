...
#绘制线框
def draw_3dframeworks(vis,points,category):
    position = points
...
if __name__ == "__main__":
    ...
    vis.create_window(width=1308 ,height=776)#创建窗口
    ...
    for obj_index in range(len(obj)):
        if obj[obj_index].name == "Car" or obj[obj_index].name == "Pedestrian" or obj[obj_index].name == "Cyclist":
            ...
            draw_3dframeworks(vis, Y, category)
    ...
    #view_control.set_zoom(10)#小数放大，整数缩小

    #json视角配置，通过鼠标调整视角以后，按下 p 就会截屏且保存一个.json文件，这个json文件里保存着视角配置.
    view_control = vis.get_view_control()
    param = o3d.io.read_pinhole_camera_parameters('5241.json')
